  ...
  <div class="container">
    <vue-scroll>
      <div class="header flex align-center">
        ...
        <div class="tab tab1">
          <div class="menuItem" :class="menuIndex===0?'menuItem2':''">1、基础信息</div>
        </div>
        <div class="tab tab2">
          <div class="menuItem" :class="menuIndex===1?'menuItem2':''">2、项目流程</div>
        </div>
      </div>
      <div class="main">

        <div v-if="menuIndex===0" class="formArea">
          <el-form ref="form" :model="form" label-width="182px">
            ...
            <div class="flex mt30">
              ...
              <div v-if="form.coverUrl!==''" class="fileImg_box2">
                <img class="fileImg2" :src="form.coverUrl" alt>
                <div class="mask">
            ...
            >
              <el-button v-waves size="medium" type="primary" @click="next">下一步</el-button>
            </div>
          </el-form>
        </div>

        <div v-if="menuIndex===1" class="flowArea">
          <div v-for="(item,index) in form.flowNodes" :key="index" class="flow">
            <div v-if="item.type===1" class="flowBox">
              <div class="flowHeader flex justify-between align-center">
                <input
                  ...
                  @focus="inputName(index)"
                  @blur="inputName2(index)"
                >
                <img
                  v-if="index!==0"
                  class="delIcon"
              ...
              <div class="executorBox flex align-center" @click="addPerson(index)">
                <div class="executor1 pointer">本环节执行人员</div>
                <div class="executor2 flex flex-wrap pointer">
                  <!-- <img v-for="item2 in 3" :key="item2" src="../../assets/images/avatar.png" alt> -->
                  <div
                    v-for="(sub,subIndex) in item.executorIds"
                    :key="subIndex"
                    class="avatar"
                  >{{ sub.nickname }}</div>
                </div>
            ...
            <div v-if="item.type===2" class="flowBox">
              ...
              <div class="executorBox flex align-center" @click="addPerson(index)">
                <div v-if="item.executorIds.length==0" class="executor4 pointer">请选择审核人员</div>
          ...
          <div class="btns flex justify-between" style="padding-bottom:50px;">
            <div v-waves class="lastStep" @click="lastStep">上一步</div>
            <div v-waves class="saveTemplate" @click="saveTemplate">保存模板</div>
          </div>
        </div>
      ...
      <el-drawer ref="drawer" :visible.sync="drawer" direction="rtl" :show-close="show_close">
        <div class="drawerHead flex align-center">
          <div class="drawerHead1">{{ form.flowNodes[addPerson_index].type===1?'环节执行人':'节点审批人' }}</div>
          <div
            v-if="form.flowNodes[addPerson_index].type===1"
            class="drawerHead2"
          >{{ form.flowNodes[addPerson_index].name }}</div>

          <div v-if="form.flowNodes[addPerson_index].type===2" class="drawerHead2">{{ lastLinkName }}</div>
          <div v-if="form.flowNodes[addPerson_index].type===2" class="ZHI">至</div>
          <div v-if="form.flowNodes[addPerson_index].type===2" class="drawerHead3">{{ nextLinkName }}</div>
        </div>
        <div class="drawer_add flex align-center">
          <div class="sureBtn" style="width:90px;margin-left:30px;" @click="showSelectForm">指定成员</div>
          <div v-if="form.flowNodes[addPerson_index].type===1" class="count">（共 {{ count }} 人）</div>
        </div>

        <div class="drawer_main">
          <div class="table-responsive mt20 mark1">
            <el-table
              :data="form.flowNodes[addPerson_index].executorIds"
              :empty-text="emptyText"
              style="width: 100%"
            >
              <el-table-column label="*" style="padding-left:51px;">
...
export default {
  ...
  methods: {
    ...
    cancelAdd() {
      // 恢复打开drawer之前的样子
      this.drawer = false
      this.form.flowNodes[this.addPerson_index].executorIds.splice(
    ...
    sureAdd() {
      ...
      ].executorIds = this.form.flowNodes[
        this.addPerson_index
      ].executorIds.reduce((item, next) => {
        if (!hash[next.id]) {
          hash[next.id] = true
          item.push(next)
        }
    ...
    delPerson(id) {
      for (
        let i = 0;
        i < this.form.flowNodes[this.addPerson_index].executorIds.length;
        i++
      ) {
        if (
          this.form.flowNodes[this.addPerson_index].executorIds[i].id === id
        ) {
          this.form.flowNodes[this.addPerson_index].executorIds.splice(i, 1)
        }
      }
    },
    showSelectForm() {
    ...
    sureSelect() {
      ...
      // reduce方法去重
      this.form.flowNodes[this.addPerson_index].executorIds = this.form.flowNodes[this.addPerson_index].executorIds.concat(this.selectedMember)
      const hash = {}
      this.form.flowNodes[this.addPerson_index ].executorIds = this.form.flowNodes[this.addPerson_index].executorIds.reduce((item, next) => {
        if (!hash[next.id]) {
          hash[next.id] = true
          item.push(next)
        }
    ...
    addPerson(index) {
      ...
      this.executor_length = this.form.flowNodes[
        this.addPerson_index
      ].executorIds.length
      for (let i = 0; i < index; i++) {
        if (this.form.flowNodes[i].type === 1) {
          this.lastLinkName = this.form.flowNodes[i].name
        }
      }
      for (let i = index; i < (this.form.flowNodes.length); i++) {
        if (this.form.flowNodes[i].type && this.form.flowNodes[i].type === 1) {
          this.nextLinkName = this.form.flowNodes[i].name
          return
        } else {
    ...
    sureDel() {
      this.delDialog = false
      this.form.flowNodes.splice(this.selectNode, 1)
    },
    delNode(index) {
    ...
    addLink(index) {
      ...
      this.form.flowNodes.splice(index + 1, 0, obj)
      this.form.flowNodes[index].visible = false
    },
    addExecutor(index) {
      const obj = {
        name: '',
    ...
    inputName(index) {
      const inputValue = this.form.flowNodes[index].name
      if (inputValue === '项目环节名称') {
        this.form.flowNodes[index].name = ''
      }
    },
    inputName2(index) {
      const inputValue = this.form.flowNodes[index].name
      if (inputValue === '') {
        this.form.flowNodes[index].name = '项目环节名称'
      }
    },
    backTemplate() {
      this.$router.push({
    ...
    handleAvatarSuccess(res) {
      this.form.coverUrl = res[0].imageUrl
    },
    delNewFileImg() {
      this.form.coverUrl = ''
    }
...
.tab1{
  margin-left:569* $bg1px;
}
.tab2 {
  margin-left:332* $bg1px;
}
.saveBtn{
  width:90* $bg1px;
height:35* $bg1px;
...
text-align: center;
line-height: 35* $bg1px;
   margin-left:610* $bg1px;
   cursor: pointer;
   @media(min-width:1800px){
     &{
       font-size: 14px;
